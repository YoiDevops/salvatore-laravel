...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    ...
    <a href="{{ route('cargas.create', request()->route('current_team')) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow">
        ...
        <tbody class="divide-y divide-gray-200 text-sm">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="p-4 space-x-2 flex items-center">
                    <a href="{{ route('cargas.edit', [request()->route('current_team'), $carga->id_carga ?? $carga->id]) }}" class="text-yellow-600 hover:text-yellow-900 font-medium">Editar</a>
                    <form action="{{ route('cargas.destroy', [request()->route('current_team'), $carga->id_carga ?? $carga->id]) }}" method="POST" onsubmit="return confirm('¿Está seguro de eliminar esta asignación?');" class="inline">
                        @csrf
                        @method('DELETE')
