...
<form action="{{ route('asignaturas.update', [request()->route('current_team'), $asignatura->id_asignatura ?? $asignatura->id]) }}" method="POST" class="bg-white p-6 rounded-lg shadow space-y-6">
    @csrf
    @method('PUT')
