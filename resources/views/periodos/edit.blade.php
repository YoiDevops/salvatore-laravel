...
<form action="{{ route('periodos.update', [request()->route('current_team'), $periodo->id_periodo ?? $periodo->id]) }}" method="POST" class="bg-white p-6 rounded-lg shadow space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="flex justify-end space-x-3 pt-4 border-t">
        <a href="{{ route('periodos.index', request()->route('current_team')) }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg">Cancelar</a>
