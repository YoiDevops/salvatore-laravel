...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    ...
    <a href="{{ route('areas.create', request()->route('current_team')) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow">
        + Registrar Área
    </a>
</div>
        ...
        <tbody class="divide-y divide-gray-200 text-sm">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="p-4 space-x-2 flex items-center">
                    <a href="{{ route('areas.edit', [request()->route('current_team'), $area->id_area ?? $area->id]) }}" class="text-yellow-600 hover:text-yellow-900 font-medium">Editar</a>
                    <form action="{{ route('areas.destroy', [request()->route('current_team'), $area->id_area ?? $area->id]) }}" method="POST" onsubmit="return confirm('¿Está seguro de eliminar esta área?');" class="inline">
                        @csrf
                        @method('DELETE')
